...
import { InvalidBundleProductsException } from 'src/bundle/domain/domain-exceptions/invalid-bundle-products-exception';
...
describe("Bundle Aggregate Invariants", () => {
  ...
  test("should not create a bundle with one products id, at least need's two products id", () => {
    try {
    ...
    assert.ok(
      caughtError instanceof InvalidBundleProductsException,
      `Expected InvalidBundleProductsException but got ${caughtError}`
    )
  })
})