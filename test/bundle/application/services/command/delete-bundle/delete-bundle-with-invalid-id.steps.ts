...
When('Trying to delete a Bundle with bundle id {string}',
  async (bundleId: string) => {
  
    const bundles: Bundle[] = [
  Bundle.initializeAggregate(
    BundleId.create('7ea54c93-562b-4c74-8b24-040add21f4c4'),
...
Then('The bundle should not be deleted because the bundle {string}, is not registered', async (bundleId: string) => {
  assert.ok(caughtError instanceof NotFoundException, `Bundle with id ${bundleId} is not registered, ca`);
})